...
export default function UploadCta() {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {state.status === "success" && <div className="text-xs text-emerald-600">Uploaded ✓</div>}
      {state.status === "error" && <div className="text-xs text-red-600">{state.message}</div>}
    </div>
  );
}

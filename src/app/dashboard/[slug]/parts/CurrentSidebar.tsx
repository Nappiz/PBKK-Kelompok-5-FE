...
export default function CurrentSidebar({ active = "summarize", onChange, userName, expanded, setExpanded }: Props) {
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 px-4 py-2 space-y-2 min-w-[280px] md:min-w-0">
          ...
          <div className="my-4 h-px bg-neutral-200/60 mx-2" />
          
          <SidebarAction 
            icon={<BookOpen size={22} />} 
            label="Summarize" 
            active={active === "summarize"} 
            onClick={() => { onChange?.("summarize"); if(isMobile) setExpanded(false); }} 
            expanded={expanded || isMobile}
          />
          <SidebarAction 
            ...
            label="AI Chat" 
            active={active === "ai"} 
            onClick={() => { onChange?.("ai"); if(isMobile) setExpanded(false); }} 
            expanded={expanded || isMobile}
          />
          <SidebarAction 
            ...
            onClick={() => { onChange?.("flashcards"); if(isMobile) setExpanded(false); }} 
            expanded={expanded || isMobile}
          />
        </nav>
          ...
          <div className="relative">
            ...
            >
              ...
              {(expanded || isMobile) && (
                ...
                >
                  <p className="text-sm font-semibold text-neutral-900 truncate font-krona">{userName}</p>
...
function SidebarItem({ icon, label, href, expanded }: any) {
  return (
    <a
      href={href}
      className={`
        cursor-pointer group flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-300 relative overflow-hidden text-neutral-500 hover:bg-white/60 hover:text-neutral-900 hover:shadow-sm
...
function SidebarAction({ icon, label, active, onClick, expanded }: any) {
  return (
    <button
      onClick={onClick}
      className={`
        cursor-pointer group flex w-full items-center gap-3 px-3 py-3 rounded-xl transition-all duration-300 relative overflow-hidden
        ${active 
